...
## Objective
Produce reproducible figures/tables based only on available measurements and documented manual observations.
...
You are the implementation agent for **Session 17 only**. Read the hardware lock and current TASKS state before editing. Produce reproducible figures/tables based only on available measurements and documented manual observations. Preserve previous accepted work. Do not start Session 18 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S17 results and figures"
```
