...
## Objective
Design a controlled small-step pH perturbation with explicit stop conditions and recovery logging.
...
- `experiments/EXP03_PH_PERTURBATION.md`
...
You are the implementation agent for **Session 15 only**. Read the hardware lock and current TASKS state before editing. Design a controlled small-step pH perturbation with explicit stop conditions and recovery logging. Preserve previous accepted work. Do not start Session 16 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S15 ph perturbation experiment"
```
