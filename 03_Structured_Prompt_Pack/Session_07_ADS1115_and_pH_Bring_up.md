...
## Objective
Validate ADS1115 0x48, read pH on A1, and implement two-point pH calibration workflow.
...
You are the implementation agent for **Session 07 only**. Read the hardware lock and current TASKS state before editing. Validate ADS1115 0x48, read pH on A1, and implement two-point pH calibration workflow. Preserve previous accepted work. Do not start Session 08 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S07 ads1115 and ph bring-up"
```
