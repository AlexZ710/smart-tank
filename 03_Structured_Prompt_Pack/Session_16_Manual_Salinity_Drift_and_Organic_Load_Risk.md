...
## Objective
Use manual salinity and manual observations/reagent data; do not claim EC or ammonia are automatically measured.
...
- `experiments/EXP04_MANUAL_SALINITY_DRIFT.md`
- `experiments/EXP05_ORGANIC_LOAD_RISK.md`
...
You are the implementation agent for **Session 16 only**. Read the hardware lock and current TASKS state before editing. Use manual salinity and manual observations/reagent data; do not claim EC or ammonia are automatically measured. Preserve previous accepted work. Do not start Session 17 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S16 manual salinity drift and organic-load risk"
```
