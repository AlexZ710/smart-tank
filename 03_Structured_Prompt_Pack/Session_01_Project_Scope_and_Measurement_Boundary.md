...
## Objective
Freeze the 5 L monitoring-only project scope. Explicitly list measured, manual, optional and unavailable variables.
...
You are the implementation agent for **Session 01 only**. Read the hardware lock and current TASKS state before editing. Freeze the 5 L monitoring-only project scope. Explicitly list measured, manual, optional and unavailable variables. Preserve previous accepted work. Do not start Session 02 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S01 project scope and measurement boundary"
```
