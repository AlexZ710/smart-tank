...
## Objective
Design a manual/external controlled temperature-response experiment without claiming automated heater control.
...
- `experiments/EXP02_TEMP_RESPONSE.md`
...
You are the implementation agent for **Session 14 only**. Read the hardware lock and current TASKS state before editing. Design a manual/external controlled temperature-response experiment without claiming automated heater control. Preserve previous accepted work. Do not start Session 15 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S14 temperature response experiment"
```
