...
## Objective
Implement robust serial CSV ingestion with optional XKC field and append-only raw data.
...
- `backend/collector/serial_collector.py`
- `data/raw/reef_data.csv`
...
You are the implementation agent for **Session 09 only**. Read the hardware lock and current TASKS state before editing. Implement robust serial CSV ingestion with optional XKC field and append-only raw data. Preserve previous accepted work. Do not start Session 10 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S09 host serial collector"
```
