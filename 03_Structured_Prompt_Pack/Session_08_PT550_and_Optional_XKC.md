...
## Objective
Validate PT550 on A3; explicitly mark XKC installed/not-installed without blocking core progress.
...
You are the implementation agent for **Session 08 only**. Read the hardware lock and current TASKS state before editing. Validate PT550 on A3; explicitly mark XKC installed/not-installed without blocking core progress. Preserve previous accepted work. Do not start Session 09 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S08 pt550 and optional xkc"
```
