...
## Goal
Add local PostgreSQL through Docker Desktop, SQL schema/migrations and repeatable local initialization.
...
docker-compose.yml, database/schema.sql, database/README.md, .env.example

## Implementation prompt
Implement only Session 20: **Docker PostgreSQL and Schema**. Reuse the existing repository and preserve all completed earlier work. Do not perform later sessions. Keep the system safe, reproducible, and evidence-driven. The web facade must distinguish current, stale, missing, optional, manual, and unavailable data rather than inventing values.
...
- [ ] Evidence saved under `evidence/S20/`.
...
## Commit message
`feat: add local PostgreSQL schema and Docker workflow`

## Stop condition
Stop after Session 20 acceptance is recorded. Do not auto-run Session 21.
