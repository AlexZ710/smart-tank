...
## Objective
Run/prepare a 24–48 h baseline using only available telemetry plus optional manual entries.
...
You are the implementation agent for **Session 13 only**. Read the hardware lock and current TASKS state before editing. Run/prepare a 24–48 h baseline using only available telemetry plus optional manual entries. Preserve previous accepted work. Do not start Session 14 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S13 baseline stability experiment"
```
