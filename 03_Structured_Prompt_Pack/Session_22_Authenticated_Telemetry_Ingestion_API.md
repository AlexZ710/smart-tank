...
## Goal
Implement validated device POST ingestion and bounded read endpoints backed by PostgreSQL.
...
web/app/api/telemetry/**, web/lib/db.ts, web/lib/validation.ts, tests or scripts

## Implementation prompt
Implement only Session 22: **Authenticated Telemetry Ingestion API**. Reuse the existing repository and preserve all completed earlier work. Do not perform later sessions. Keep the system safe, reproducible, and evidence-driven. The web facade must distinguish current, stale, missing, optional, manual, and unavailable data rather than inventing values.
...
- [ ] Evidence saved under `evidence/S22/`.
...
## Commit message
`feat: add authenticated telemetry ingestion API`

## Stop condition
Stop after Session 22 acceptance is recorded. Do not auto-run Session 23.
