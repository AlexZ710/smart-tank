...
## Objective
Validate DS18B20 standalone serial output and record evidence.
...
You are the implementation agent for **Session 06 only**. Read the hardware lock and current TASKS state before editing. Validate DS18B20 standalone serial output and record evidence. Preserve previous accepted work. Do not start Session 07 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S06 ds18b20 temperature bring-up"
```
