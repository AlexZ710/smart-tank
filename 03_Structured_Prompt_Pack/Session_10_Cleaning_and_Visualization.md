...
## Objective
Clean missing/invalid readings while preserving raw data and visualize temperature, pH and relative light.
...
- `backend/visualization/visualize.py`
- `backend/collector/clean_data.py`
...
You are the implementation agent for **Session 10 only**. Read the hardware lock and current TASKS state before editing. Clean missing/invalid readings while preserving raw data and visualize temperature, pH and relative light. Preserve previous accepted work. Do not start Session 11 unless this is Session 18. Update documentation when behavior/schema changes. Keep the project runnable if optional XKC hardware is absent.
...
```bash
git add .
git commit -m "S10 cleaning and visualization"
```
