...
- Post-S28 deployment (2026-09-19, DONE): site LIVE on Vercel + Neon. Local Postgres migrated to Neon (`late-surf-75594067`, branch `production`, schema.sql 5 tables applied, TLS via web/lib/db.ts rejectUnauthorized). GitHub repo `AlexZ710/smart-tank` pushed (all commits; repo-local proxy http://127.0.0.1:51081). Vercel project `smart-tank` (prj_47i5keeMpV7I2ihwF3mD1xrE6tfK, Root Directory `web`, framework `nextjs`) connected to GitHub master; production alias **https://smart-tank-one.vercel.app**. Deploy driven via Vercel REST API (CLI login fetch broken through host proxy). 7 server-side env vars set (DATABASE_URL pooled Neon, DEVICE_INGEST_TOKEN, REPORTS_GENERATE_TOKEN, AI_PROVIDER/AI_API_KEY/AI_MODEL/AI_BASEURL) — production secrets live ONLY in Vercel env + operator gitignored .env.local + C:\Users\Alex1\smart-tank-vercel-env.txt (never committed/printed). LIVE acceptance captured: /api/health {"status":"ok","database":"up"}; security headers present (CSP/HSTS/DENY/nosniff/permissions-policy) + x-powered-by absent; six pages 200; telemetry no/wrong token 401, forbidden field orp_mv 400, non-vocabulary rule_code=ORP_HIGH 400; reports/generate no token 401, empty window w/ admin token 200 no-data-shortcut (provider NOT called); labeled smoke ingest device `e2e-smoke-test` (2 rows) accepted:2 -> read back -> DELETEd from Neon -> re-verified 0 rows everywhere; response secret scans (AI-key prefix/host/scheme probes + device/admin tokens) 0 hits. Empty commit 19ec1d9 pushed to trigger the git build. STILL DEFERRED (unchanged, hardware/account-gated): board-attached provisioning runtime, real-tank EXP01-EXP05, pH buffer calibration. NEXT: connect ESP32-S3 sensors -> PROVISION serial command with api_url=https://smart-tank-one.vercel.app/api/telemetry + DEVICE_INGEST_TOKEN (value in C:\Users\Alex1\smart-tank-vercel-env.txt) -> real telemetry flows to Neon and shows live on the dashboard.
